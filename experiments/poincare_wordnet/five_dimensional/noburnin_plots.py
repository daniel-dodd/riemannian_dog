...
with open(os.path.join(dir_path, "results/noburnin_mammals.pkl"), "rb") as f:
    experiment_data_all = pickle.load(f)

with open(
    os.path.join(dir_path, "results/noburnin_mammals_productwise.pkl"), "rb"
) as f:
...
for WHICH in ["sensitivity_noc", "sensitivity"]:
    for m in METRICS:
        ...
        for opt in mean["learning_rate"]:
            if opt == "radam":
                ...
                ax.fill_between(
                    ...
                    [
                        mean_pw["learning_rate"][opt][_]["iterate"][m]
                        + standard_error_pw["learning_rate"][opt][_]["iterate"][m]
                        for _ in range(len(LEARNING_RATES))
                    ],
                    alpha=0.15,
                    color=OPTIMS_TO_PLOT[opt]["color"],
        ...
        # Save the figure.
        plt.savefig(os.path.join(dir_name, f"noburnin_mammals_{WHICH}.pdf"))
