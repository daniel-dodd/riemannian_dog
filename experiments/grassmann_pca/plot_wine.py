...
dir_name = os.path.join(dir_path, "results/wine.pkl")

with open(dir_name, "rb") as f:
    experiment_data_all = pickle.load(f)

# Create plots directory.
dir_name = os.path.join(dir_path, "plots_wine")
...
for ITERATION in [100, 500, 1000, 2000, 5000]:
    for m in METRICS:
        ...
        # Save the figure.
        plt.savefig(os.path.join(dir_name, f"wine_{m}_{ITERATION}.pdf"))
...
for METRIC in METRICS:
    for WHICH in ["rdowg", "rdog", "nrdog"]:
        ...
        fig.tight_layout()
        plt.savefig(
            os.path.join(dir_name, f"wine_sensitivity_trace_{WHICH}_{METRIC}.pdf")
        )
