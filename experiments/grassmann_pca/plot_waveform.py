...
dir_name = os.path.join(dir_path, "results/waveform.pkl")

with open(dir_name, "rb") as f:
    experiment_data_all = pickle.load(f)

# Create plots directory.
dir_name = os.path.join(dir_path, "plots_waveform")
...
METRICS = {
    "regret": dict(label=r"Regret", yscale="linear"),
    ...
    for m in METRICS:
        ...
        for opt in mean["learning_rate"]:
            ax.plot(
                LEARNING_RATES,
                [
                    mean["learning_rate"][opt][_]["average"][m][ITERATION]
            ...
            ax.fill_between(
                ...
                [
                    mean["learning_rate"][opt][_]["average"][m][ITERATION]
                    + standard_error["learning_rate"][opt][_]["average"][m][ITERATION]
                    for _ in range(len(LEARNING_RATES))
                ],
                alpha=0.15,
                color=OPTIMS_TO_PLOT[opt]["color"],
        ...
        # Save the figure.
        plt.savefig(os.path.join(dir_name, f"waveform_{m}_{ITERATION}.pdf"))
...
for METRIC in METRICS:
    for WHICH in ["rdowg", "rdog", "nrdog"]:
        ...
        fig.tight_layout()
        plt.savefig(
            os.path.join(dir_name, f"waveform_sensitivity_trace_{WHICH}_{METRIC}.pdf")
        )
