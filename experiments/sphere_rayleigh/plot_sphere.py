...
dir_name = os.path.join(dir_path, "results/sphere.pkl")
...
for ITERATION in [100, 500, 1000, 2000, 5000]:
    for m in METRICS:
        ...
        # Save the figure.
        plt.savefig(os.path.join(dir_name, f"sphere_{m}_{ITERATION}.pdf"))
...
for METRIC in METRICS:
    for WHICH in ["rdowg", "rdog", "nrdog"]:
        ...
        fig.tight_layout()
        plt.savefig(
            os.path.join(dir_name, f"sphere_sensitivity_trace_{WHICH}_{METRIC}.pdf")
        )
...
for METRIC in METRICS:
    for WHICH in ["radam", "rsgd"]:
        ...
        # plt.colorbar(cm.ScalarMappable(norm=normalize, cmap=cmap), ax=ax)  # Add a colorbar
        cbar = plt.colorbar(
            cm.ScalarMappable(norm=normalize, cmap=cmap), ax=ax, rasterized=True
        )
